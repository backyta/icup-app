...
import { getOfferingsIncomeProportion } from '@/modules/metrics/services';
...
export const OfferingIncomeProportionCard = ({ churchId }: Props): JSX.Element => {
  ...
  //* Queries
  const { data } = useQuery({
    queryKey: ['offerings-income-proportion', churchId],
    queryFn: () =>
      getOfferingsIncomeProportion({
        searchType: MetricSearchType.OfferingsIncomeByProportion,
