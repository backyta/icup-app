...
export const OfferingIncomeAnalysisCardBySpecialOffering = ({ churchId }: Props): JSX.Element => {
  //* States
  ...
  const offeringIncomeBySpecialOffering = useQuery({
    queryKey: ['offering-income-by-special-offering', { ...searchParams, church: churchId }],
    queryFn: () => {
    ...
    <Card className='bg-slate-50/40 dark:bg-slate-900/40 flex flex-col col-start-1 col-end-2 h-[24rem] md:h-[25rem] lg:h-[26rem] 2xl:h-[26rem] m-0 border-slate-200 dark:border-slate-800'>
      <CardHeader className='z-10 flex flex-col sm:flex-row items-center justify-between px-4 py-2.5'>
        <CardTitle className='flex justify-center items-center gap-2 font-bold text-[22px] sm:text-[25px] md:text-[28px] 2xl:text-[30px]'>
          ...
          {offeringIncomeBySpecialOffering?.data &&
            Object.entries(offeringIncomeBySpecialOffering?.data)?.length > 0 && (
              <Badge
                variant='active'
      ...
      {!offeringIncomeBySpecialOffering?.data?.length && !searchParams ? (
        ...
        <CardContent className='h-full px-2 sm:px-4 py-0'>
          {offeringIncomeBySpecialOffering?.isFetching &&
          ...
          {!year && !offeringIncomeBySpecialOffering?.data?.length && (
