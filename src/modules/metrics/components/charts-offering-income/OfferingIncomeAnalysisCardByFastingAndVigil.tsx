...
const renderTooltipContent = (o: any): JSX.Element => {
  ...
  return (
    <div className='bg-white p-2 text-black font-normal'>
      <p className=''>{`${label}`}</p>
      <p className=''>{`${payload[0]?.payload?.Tipo}`}</p>
      {payload[0]?.payload?.Zona && <p className=''>{`Zona: ${payload[0]?.payload?.Zona}`}</p>}
      <ul className='list'>
...
export const OfferingIncomeAnalysisCardByFastingAndVigil = (): JSX.Element => {
  //* States
    ...
    <Card className='bg-slate-50/40 dark:bg-slate-900/40 flex flex-col col-start-2 col-end-3 h-[22rem] lg:h-[25rem] 2xl:h-[26rem] m-0 border-slate-200 dark:border-slate-800'>
      ...
      >
        ...
        >
          ...
          <Area type='monotone' dataKey='Cantidad' stroke='#cb9903' fill='#cb9903' />
        </AreaChart>
      </ResponsiveContainer>
    </Card>
  );
