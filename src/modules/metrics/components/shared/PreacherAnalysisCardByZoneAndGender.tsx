...
import { getPreachersByZoneAndGender } from '@/modules/metrics/services';
import { Card, CardContent, CardHeader, CardTitle } from '@/shared/components/ui/card';

import { PreachersByZoneAndGenderTooltipContent } from '@/modules/metrics/components/charts-member/tooltips/components';
...
export const PreacherAnalysisCardByZoneAndGender = (): JSX.Element => {
  ...
  const membersByZoneAndGenderQuery = useQuery({
    queryKey: ['preachers-by-zone-and-gender', searchParams],
    queryFn: async () => {
      return await getPreachersByZoneAndGender({
        ...
        <CardContent className='h-full pl-3 pr-6 py-0'>
          {!!mappedData?.length && searchParams ? (
            <ChartContainer
              config={chartConfig}
              className={cn(
                'w-full h-[258px] sm:h-[288px] md:h-[290px] lg:h-[335px] xl:h-[332px] 2xl:h-[350px]'
              )}
            >
              <AreaChart
                accessibilityLayer
