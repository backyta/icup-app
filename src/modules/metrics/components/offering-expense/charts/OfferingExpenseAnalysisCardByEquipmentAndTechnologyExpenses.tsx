...
export const OfferingExpenseAnalysisCardByEquipmentAndTechnologyExpenses = ({
  churchId,
}: Props): JSX.Element => {
  //* States
  ...
  const equipmentAndTechnologyOfferingExpenses = useQuery({
    queryKey: ['equipment-and-technology-offering-expenses', { ...searchParams, church: churchId }],
    queryFn: () => {
  ...
  useEffect(() => {
    if (equipmentAndTechnologyOfferingExpenses?.data) {
      const transformedData = equipmentAndTechnologyOfferingExpenses?.data.map(
        (offeringExpense) => {
  ...
  return (
    <Card className='bg-slate-50/40 dark:bg-slate-900/40 flex flex-col col-start-2 col-end-3 h-[22rem] md:h-[28rem] lg:h-[26rem] 2xl:h-[26rem] m-0 border-slate-200 dark:border-slate-800'>
      <CardHeader className='z-10 flex flex-col sm:flex-row items-center justify-between px-4 py-2.5'>
        <CardTitle className='flex justify-center items-center gap-2 font-bold text-[22px] sm:text-[25px] md:text-[28px] 2xl:text-[30px]'>
          {intermediateMD ? (
            <span>Gastos Equipamiento y Tecnología</span>
          ) : (
            <span>Gastos Equip./Tecnología</span>
          )}
          {equipmentAndTechnologyOfferingExpenses?.data &&
            Object.entries(equipmentAndTechnologyOfferingExpenses?.data)?.length > 0 && (
              <Badge
                variant='active'
      ...
      {!equipmentAndTechnologyOfferingExpenses?.data?.length && !searchParams ? (
        ...
        <CardContent className='h-full pl-3 pr-6 py-0'>
          {equipmentAndTechnologyOfferingExpenses?.isFetching &&
            !equipmentAndTechnologyOfferingExpenses?.data?.length &&
            year && (
              <div className='text-blue-500 text-[14px] md:text-lg flex flex-col justify-center items-center h-full -mt-6'>
                <FcDataBackup className='text-[6rem] pb-2' />
                <p>Consultando datos....</p>
