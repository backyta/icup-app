...
const dataJanuary = [
  ...
  {
    SubTipo: 'Serv. Pintura y Retoques',
    ExpenseOne: {
      date: '03/01/24',
      amount: 48.6,
    },
  ...
  {
    SubTipo: 'Serv. Limpieza profunda',
    ExpenseOne: {
      date: '06/01/24',
      amount: 57.3,
    },
  ...
  {
    SubTipo: 'Rep. Mant. Equ. Sonido e Ilum.',
    ExpenseOne: {
      date: '07/01/24',
      amount: 49.8,
    },
  ...
  {
    SubTipo: 'Mant. Jardín y Exteriores',
    ExpenseOne: {
      date: '08/01/24',
      amount: 53.6,
    },
  ...
  {
    SubTipo: 'Rep. Mant Equ. Informáticos',
    ExpenseOne: {
      date: '11/01/24',
      amount: 48.7,
    },
  ...
  {
    SubTipo: 'Rep. de techos y estruc.',
    ExpenseOne: {
      date: '12/01/24',
      amount: 52.4,
    },
...
const renderTooltipContent = (o: any): JSX.Element => {
  ...
  return (
    <div className='bg-white p-2 text-black font-normal'>
      <p className=''>{`${label}`}</p>
      <p>
        <span className='text-slate-500'>{`${payload[0]?.payload?.ExpenseOne?.date}`} - </span>
        <span className='text-slate-500'>{`${payload[0]?.payload?.ExpenseOne?.amount} soles`}</span>
      </p>
      <p>
        <span className='text-slate-500'>{`${payload[0]?.payload?.ExpenseTwo?.date}`} - </span>
        <span className='text-slate-500'>{`${payload[0]?.payload?.ExpenseTwo?.amount} soles`}</span>
      </p>
      <p>
        <span className='text-slate-500'>{`${payload[0]?.payload?.ExpenseThree?.date}`} - </span>
        <span className='text-slate-500'>{`${payload[0]?.payload?.ExpenseThree?.amount} soles`}</span>
      </p>
      <p>
        <span className='text-slate-500'>{`${payload[0]?.payload?.ExpenseFour?.date}`} - </span>
        <span className='text-slate-500'>{`${payload[0]?.payload?.ExpenseFour?.amount} soles`}</span>
      </p>
      <ul className='list'>
...
export const OfferingExpensesAnalysisCardByMaintenanceAndRepairExpenses = (): JSX.Element => {
  //* States
  ...
  //* Form handler
  const handleSubmit = (values: z.infer<typeof metricsFormSchema>): void => {
    console.log({ values });
  };
    ...
    <Card className='bg-slate-50/40 dark:bg-slate-900/40 flex flex-col col-start-2 col-end-3 h-[22rem] lg:h-[25rem] 2xl:h-[26rem] m-0 border-slate-200 dark:border-slate-800'>
      ...
      >
        ...
        >
          ...
          <Area type='monotone' dataKey='Total' stroke='#3664fb' fill='#3664fb' />
        </AreaChart>
      </ResponsiveContainer>
    </Card>
  );
