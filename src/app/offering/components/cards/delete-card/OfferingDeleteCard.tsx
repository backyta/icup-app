...
import { formDeleteOffering } from '@/app/offering/validations';
...
import { ReasonTypeNames } from '@/app/offering/enums/reason-type.enum';
...
export const OfferingDeleteCard = (): JSX.Element => {
  ...
  //* Form
  const form = useForm<z.infer<typeof formDeleteOffering>>({
    mode: 'onChange',
    resolver: zodResolver(formDeleteOffering),
    defaultValues: {
      reasonType: '',
      date: undefined,
      userID: '',
  ...
  //* Form handler
  const handleSubmit = (values: z.infer<typeof formDeleteOffering>): void => {
    ...
    <Dialog open={isCardOpen} onOpenChange={setIsCardOpen}>
      ...
      <DialogContent className='w-[23rem] sm:w-[25rem] md:w-full'>
        <div className='h-auto'>
          ...
          <p className='h-[21rem] md:h-[18rem]'>
            ...
            <span className='w-full text-left inline-block mb-2 text-[14px] md:text-[15px]'>
              ❌ El registro de esta Ofrenda se colocara en estado{' '}
              <span className='font-bold'>INACTIVO.</span>
            </span>
            <span className='w-full text-left inline-block mb-2 text-[14px] md:text-[15px]'>
              ❌ El registro de esta Ofrenda se eliminara de los lugares donde guardaba relación con
              Casa Familiar, Zona, Discípulo (Miembro), Co-Pastor, Predicador o Supervisor.
            </span>
            <span className='w-full text-left inline-block mb-2 text-[14px] md:text-[15px]'>
                  ...
                  return (
                    <FormItem className='mb-4 mt-4'>
                      ...
                      <Select value={field.value} onValueChange={field.onChange}>
                        ...
                        <SelectContent>
                          {Object.entries(ReasonTypeNames).map(([key, value]) => (
