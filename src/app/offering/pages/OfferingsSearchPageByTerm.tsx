import { offeringInfoColumns } from '@/app/offering/components';

import { dataOfferings as data } from '@/shared/data';
import { DataTableSearchByTerm } from '@/shared/components';

export const OfferingsSearchPageByTerm = (): JSX.Element => {
  return (
    <div>
      ...
      <p className='dark:text-slate-300 text-left font-sans font-bold px-4 sm:px-2 text-[12.5px] md:text-[15px] xl:text-base xl:px-[2rem] 2xl:px-24'>
        Busca, filtra u ordena los registros de ofrendas que necesites.
      </p>

      <div className='px-4 md:-px-2 lg:px-[2rem] xl:px-[3rem] 2xl:px-36 py-4 w-full'>
        <DataTableSearchByTerm columns={offeringInfoColumns} data={data} />
      </div>
    </div>
  );
};
