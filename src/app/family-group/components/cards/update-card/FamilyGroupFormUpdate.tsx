...
import { type FamilyGroupDataKeys, type FamilyGroupData } from '@/app/family-group/interfaces';
import { formFamilyGroupSchema } from '@/app/family-group/validations';
import { preachers, zones } from '@/app/family-group/data';
...
export const FamilyGroupFormUpdate = ({ onSubmit, onScroll }: Props): JSX.Element => {
  //* States
  ...
  //* Form
  const form = useForm<z.infer<typeof formFamilyGroupSchema>>({
    mode: 'onChange',
    resolver: zodResolver(formFamilyGroupSchema),
    defaultValues: {
      zoneName: '',
      theirPreacher: '',
      houseName: '',
  ...
  //* Handler form
  const handleSubmit = (values: z.infer<typeof formFamilyGroupSchema>): void => {
    console.log({ values });
  };
  ...
  useEffect(() => {
    for (const key in data) {
      form.setValue(key as FamilyGroupDataKeys, data[key as FamilyGroupDataKeys]);
    }
  }, []);
