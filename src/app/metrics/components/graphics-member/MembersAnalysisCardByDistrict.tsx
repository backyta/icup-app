...
export const MembersAnalysisCardByDistrict = (): JSX.Element => {
  //* States
  ...
  const totalCantidad = dataResult.reduce((total, item) => total + item.Miembros, 0);
  const newData = dataResult.map((item) => ({
    ...item,
    Porcentaje: ((item.Miembros / totalCantidad) * 100).toFixed(1),
  }));
    ...
    <Card className='bg-slate-50/40 dark:bg-slate-900/40 flex flex-col col-start-1 col-end-2 h-[22rem] lg:h-[25rem] 2xl:h-[26rem] m-0 border-slate-200 dark:border-slate-800'>
      ...
      >
        ...
        >
          ...
          <Legend />
          <Bar dataKey='Miembros' fill='#62d723' />
