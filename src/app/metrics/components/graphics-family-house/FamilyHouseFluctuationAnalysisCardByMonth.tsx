...
const dataMembersByMonth = [
  ...
  { date: 'Febrero', Nuevas: 12, Bajas: 0 },
  { date: 'Marzo', Nuevas: 11, Bajas: 1 },
  { date: 'Abril', Nuevas: 20, Bajas: 5 },
  { date: 'Mayo', Nuevas: 5, Bajas: 10 },
  { date: 'Junio', Nuevas: 2, Bajas: 5 },
  { date: 'Julio', Nuevas: 6, Bajas: 0 },
];

export const FamilyHouseFluctuationAnalysisCardByMonth = (): JSX.Element => {
  //* States
  ...
  //* Form handler
  function onSubmit(values: z.infer<typeof chartFormValidationSchema>): void {
    console.log({ values });
  }
  ...
  return (
    <Card className='bg-slate-50/40 dark:bg-slate-900/40 flex flex-col col-start-1 col-end-2 h-[22rem] lg:h-[25rem] 2xl:h-[26rem] m-0 border-slate-200 dark:border-slate-800'>
      ...
      <ResponsiveContainer
        width='100%'
        height={
          form.formState.errors.date?.message && isDesktop
            ? '75%'
            : form.formState.errors.date?.message && !isDesktop
              ? '60%'
              : isDesktopXL
      ...
      >
        ...
        >
          ...
          {!form.formState.errors.date?.message && <Legend wrapperStyle={{ left: 0 }} />}

          <Bar dataKey='Nuevas' fill='#22C55E' />
