import { UrbanSectorNames } from "@/shared/enums";

//* Allowed Urban Sector by module

// module members = disciple - pastors - copastors - leaders
export const UrbanSectorsIndependenciaAllowed = [
 // Independencia
 UrbanSectorNames.Independencia,
 UrbanSectorNames.Ermitaño,
 UrbanSectorNames.Tahuantinsuyo,
 UrbanSectorNames.Payet,
 UrbanSectorNames.Unificada,
 UrbanSectorNames.Industrial,
];

export const UrbanSectorsPuentePiedraAllowed = [
  UrbanSectorNames.Ensenada,
  UrbanSectorNames.Laderas,
  UrbanSectorNames.Chillón,
  UrbanSectorNames.Shangri_La,
  UrbanSectorNames.Tambo_Inga_Oeste,
  UrbanSectorNames.Tambo_Inga_Este,
  UrbanSectorNames.Pampa_Libre,
  UrbanSectorNames.Gallinazos,
  UrbanSectorNames.Santa_Rosa,
  UrbanSectorNames.Cercado,
  UrbanSectorNames.Las_Vegas,
  UrbanSectorNames.La_Grama,
  UrbanSectorNames.Copacabana,
  UrbanSectorNames.El_Dorado,
  UrbanSectorNames.Leoncio_Prado,
  UrbanSectorNames.Jerusalén,
  UrbanSectorNames.Lomas,
];
